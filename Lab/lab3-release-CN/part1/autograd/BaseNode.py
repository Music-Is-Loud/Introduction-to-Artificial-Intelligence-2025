...
class Attention(Node):
    ...
    def cal(self, X):
        ...
        k = X @ W_k
        v = X @ W_v #(B,L,d)
        score = q @ k.transpose(0,2,1) / np.sqrt(feat_dim)#(B,L,L)
        exp_score = np.exp(score-np.max(score,axis=2,keepdims=True))#预处理，不然容易爆上限
        softmax_score = exp_score/np.sum(exp_score,axis=2,keepdims=True)#(B,L,L)
        self.cache.append((X,softmax_score,q,k,v))
        final_score = softmax_score @ v + X
        return final_score
...
class ResLinear(Node):
    ...
    def backcal(self, grad):
        ...
        self.grad.append(extend_grad.sum(axis=0))#bias的偏导，1*20
        extend_res = np.dot(extend_grad, self.params[0].T)#x的偏导，展开，15*10
        return extend_res.reshape(*X.shape) + grad #还原回3*5*10
        raise NotImplementedError
