...
class NaiveBayesModel:
    ...
    def __call__(self, text):
        ...
        label_p[0] += math.log(self.pos_neg_num[0])-math.log(total_num)
        label_p[1] += math.log(self.pos_neg_num[1])-math.log(total_num)
        for token in text:
            for label in [0,1]:
                if token in self.token_num[label]:
                    label_p[label] += math.log(self.token_num[label][token]+alpha)
                else:
                    label_p[label] += math.log(alpha)
                label_p[label] -= math.log(self.pos_neg_num[label]+self.V*alpha)
        return np.argmax(label_p)
...
class Embedding():
    ...
    def __call__(self, text, max_len=50):
        ...
        embedded_text = np.zeros((max_len,100))
        token_num = 0
        for token in text:
            if token_num > max_len:
                break
            if token in self.emb:
                embedded_text[token_num] = self.emb[token]
            token_num+=1
...
class QAModel():
    ...
    def tf(self, word, document):
        ...
        cleaned_tokens = self.document_list[document]['document']
        tf = cleaned_tokens.count(word) / len(cleaned_tokens)
        return tf
        raise NotImplementedError  
    ...
    def __call__(self, query):
        ...
        for document in range(D):
            ...
            document_score.append(sum_tfidf)
        best_document = np.argmax(document_score)
        sentence_score=[]
        for (cleaned_tokens,sentence) in self.document_list[best_document]['sentences']:
            sum_idf = 0.0
            for word in query:
                if word in cleaned_tokens:
                    sum_idf += self.idf(word)
            sentence_score.append(sum_idf)
        best_sentence = np.argmax(sentence_score)
        ans_sentence = self.document_list[best_document]['sentences'][best_sentence][1]
        return ans_sentence
        raise NotImplementedError
