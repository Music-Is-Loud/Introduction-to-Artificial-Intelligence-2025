...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent):
            ...
            "*** YOUR CODE HERE ***"
            for nextAction in state.getLegalActions(index_of_agent):
                nextState=state.generateSuccessor(index_of_agent, nextAction)
                newValue=minimizer(nextState,depth,1)[0]
                if newValue>value:
                    value=newValue
                    maxiAction=nextAction
        ...
        def minimizer(state, depth, index_of_agent):
            ...
            for nextAction in state.getLegalActions(index_of_agent):
                nextState=state.generateSuccessor(index_of_agent, nextAction)
                if index_of_agent==gameState.getNumAgents()-1:#最后一个鬼，下一个是max
                    newValue=maximizer(nextState,depth-1,0)[0]
                else:
                    newValue=minimizer(nextState,depth,index_of_agent+1)[0]
                if newValue<value:
                    value=newValue
                    miniAction=nextAction
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent, alpha, beta):
            ...
            for nextAction in state.getLegalActions(index_of_agent):
                nextState=state.generateSuccessor(index_of_agent, nextAction)
                newValue=minimizer(nextState,depth,1,alpha,beta)[0]
                if newValue>value:
                    value=newValue
                    maxiAction=nextAction
                if value>beta:#剪枝，去掉=会导致少减，但是能确保一定选择同一条路径
                    return(value,maxiAction)
        ...
        def minimizer(state, depth, index_of_agent, alpha, beta):
            ...
            for nextAction in state.getLegalActions(index_of_agent):
                nextState=state.generateSuccessor(index_of_agent, nextAction)
                if index_of_agent==gameState.getNumAgents()-1:#最后一个鬼，下一个是max
                    newValue=maximizer(nextState,depth-1,0,alpha,beta)[0]
                else: 
                    newValue=minimizer(nextState,depth,index_of_agent+1,alpha,beta)[0]
                if newValue<value:
                    value=newValue
                    miniAction=nextAction
                if value<alpha:#剪枝，去掉=会导致少减，但是能确保一定选择同一条路径
                    return(value,miniAction)
                beta=min(value,beta)
            return (value, miniAction) 
...
class MCTSAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState, mcts_time_limit=10):
        ...
        def Selection(cgs, cgstree):
            #（1）如果当前cgstree节点还有没有被扩展的方向，也就是当前节点还需要被扩展，则返回当前gamestate和这个节点本身
            #（2）如果当前cgstree节点的所有方向都被扩展了，那么就用UCT算法找到一个最好的子节点选择，再对这个子节点重复（1）
            #什么时候结束选择？一个节点需要被扩展或者已经到达最终状态
            '''
            ...
            while cgstree.statevalue.isWin() is False and cgstree.statevalue.isLose() is False:
                ...
                for nextAction in legal_actions:
                    if(nextAction=='North' and cgstree.north is None) or (nextAction=='South' and cgstree.south is None) \
                    or(nextAction=='East' and cgstree.east is None) or (nextAction=='West' and cgstree.west is None) \
                    or (nextAction=='Stop' and cgstree.stop is None):#直接返回未被扩展的节点
                        return(cgs,cgstree)
                    if nextAction=='North':
                ...
                temp=best_UCT(children)
                cgstree=eval("cgstree."+temp[1].lower())
                cgs=temp[0]
        ...
        def Expansion(cgstree):
            ...
            for nextAction in legal_actions:
                if(nextAction=='North' and cgstree.north is None) or (nextAction=='South' and cgstree.south is None) \
                    or(nextAction=='East' and cgstree.east is None) or (nextAction=='West' and cgstree.west is None) \
                    or (nextAction=='Stop' and cgstree.stop is None):#说明这个方向要扩展
                    nextState=cgstree.statevalue.generateSuccessor(0,nextAction)#新节点的状态
                    nextNode=Node((nextState,0,1))#创建新的节点
                    nextNode.parent=cgstree#链接父节点
                    if nextAction=='North':#扩展的东西补上去
                        cgstree.north=nextNode
                    if nextAction=='South':
                        cgstree.south=nextNode
                    if nextAction=='West':
                        cgstree.west=nextNode
                    if nextAction=='East':
                        cgstree.east=nextNode   
                    if nextAction=='Stop':
                        cgstree.stop=nextNode
        ...
        def Simulation(cgs, cgstree):
            ...
            '''
            simulation_score=HeuristicFunction(cgstree.statevalue)#用那个最终随机游走到的节点对应的游戏状态去评估
        ...
        def Backpropagation(cgstree, simulation_score):
            while cgstree.parent is not None:
                ...
                '''
                cgstree=cgstree.parent#转换为这点的父节点
                cgstree.denominator+=1#这点的次数+1
                cgstree.numerator+=simulation_score#这点的分数和加上模拟分数  
            return cgstree#返回根节点
        ...
        def endSelection(cgstree):#在所有值都标好的情况下，进行最终选择
            children = []
        ...
        for _ in range(mcts_time_limit):
            gameState, cgstree = Selection(gameState, cgstree)#更新后，这是待扩展的节点。根据当前的游戏状态和搜索树，选择一个最好的子节点
            Expansion(cgstree)#扩展该节点并链接父节点，扩展这个选到的节点
