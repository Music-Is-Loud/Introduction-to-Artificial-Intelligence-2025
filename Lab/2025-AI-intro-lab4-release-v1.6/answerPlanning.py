...
class RRT:
    ...
    def find_path(self, current_position, next_food):
        ...
        ### 你的代码 ###      
        if self.map.checkline(list(current_position),list(np.array(next_food).astype(float)))==(False,None):
            self.path = [next_food]
            return
        ### 你的代码 ###
        # 如有必要，此行可删除
    ...
    def get_target(self, current_position, current_velocity):
        ...
        ### 你的代码 ###
        if self.called_times >= CALL_TIME:
        # 先检查是否到达路径末尾
            if self.current_idx >= len(self.path) - 1:  # 已是最后一个节点
                self.find_path(current_position, self.path[-1])  # 重新规划路径
                self.current_idx = 0  # 重置索引
            else:
                self.current_idx += 1
            self.called_times = 0  
        self.called_times += 1
    ...
    def build_tree(self, start, goal):
        ...
        ### 你的代码 ###
        max_x = np.max(self.walls[:,0])
        min_x = np.min(self.walls[:,0])
        max_y = np.max(self.walls[:,1])
        min_y = np.min(self.walls[:,1])
        while(1):
            uniform_point = np.array([np.random.uniform(min_x,max_x),np.random.uniform(min_y,max_y)])
            nearest_idx,nearest_distance = self.find_nearest_point(uniform_point,graph)
            is_empty,newpoint = self.connect_a_to_b(graph[nearest_idx].pos,uniform_point)
            if is_empty:#ok
                new_point=TreeNode(nearest_idx,newpoint[0],newpoint[1])
                graph.append(new_point)   
                #判断到终点
                distance_to_goal = np.sqrt((newpoint[0]-goal[0])**2+(newpoint[1]-goal[1])**2)
                if distance_to_goal < TARGET_THREHOLD or self.map.checkline(list(newpoint),list(np.array(goal).astype(float)))==(False,None):
                    #还原路径
                    path.append((goal[0],goal[1]))
                    now_point = new_point
                    while (now_point.parent_idx!=-1):
                        path.append((now_point.pos[0],now_point.pos[1]))
                        now_point=graph[now_point.parent_idx]
                    path.append((now_point.pos[0],now_point.pos[1]))
                    path = path[::-1]
                    #path.reverse()
                    break
    ...
    def find_nearest_point(point, graph):
        ...
        for i,node in enumerate(graph):
            dis = np.sqrt((node.pos[0]-point[0])**2+(node.pos[1]-point[1])**2)
    ...
    def connect_a_to_b(self, point_a, point_b):
        ...
        newpoint = np.zeros(2)
        ### 你的代码 ###
        newpoint = point_a + STEP_DISTANCE*(point_b-point_a)
        if self.map.checkline(list(point_a),list(newpoint))==(False,None):
            is_empty = True
        ### 你的代码 ###
