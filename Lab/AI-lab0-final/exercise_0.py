...
def b2(scores:dict):
    ...
    #scores:dict是类型注解，表示scores预期是一个字典
    ret = None
    for keys in scores:
        if scores[keys]>50:
            if ret==None:
...
def q2(n):
    ...
    '''
    col,row=np.ogrid[:n,:n]
    #ogrid生成两个二维数组，一个列数组一个行数组
    x = balabala
    x=col+row
    return x
...
def q3(name_score_dict):
    ...
    for i in name_score_dict:
        res.append(Student(i,name_score_dict[i]))
...
def q5():
    ...
    pi=round(4*in_circle/1000000,5)
    res=str(pi)
    res=float(res)
    return res
...
class Layout:
    ...
    def recover_layoutText(self):
        ...
        """
        # 你的实现代码
        tmpLayoutText=[[' 'for i in range(self.width)]for j in range(self.height)]
        for tur in self.walls:
            tmpLayoutText[tur[0]][tur[1]]='%'
        for pos in self.foods:
            tmpLayoutText[pos[0]][pos[1]]='.'
        if self.pacman_pos:
            tmpLayoutText[self.pacman_pos[0]][self.pacman_pos[1]]='P'
        LayoutText=""
...
def q6(file_path):
    ...
    '''
    LayoutMap=Layout.build_layout(file_path)
    return LayoutMap