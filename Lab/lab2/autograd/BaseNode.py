...
class Linear(Node):
    ...
    def __init__(self, indim, outdim):
        ...
        @param outdim: 输出维度
        """
        weight = kaiming_uniform(indim, outdim)#初始化神经网络的权重
        bias = zeros(outdim)
        super().__init__("linear", weight, bias)#params[0]存weight,params[1]存偏置
    ...
    def backcal(self, grad):
        ...
        ''' 
        # TODO: YOUR CODE HERE
        # X可能不是二维的！！（例如3样本，5长，10特征）
        # 上游梯度也可能不是二维的！！（例如3样本，5长，20特征）
        X = self.cache[-1]
        #grad_X = grad.dot(self.params[0].T)
        extend_X = X.reshape(-1, X.shape[-1])#展平为二维向量，例如15个样本10特征
        extend_grad = grad.reshape(-1, grad.shape[-1]) #上游梯度展平为二维向量，例如15个样本20特征
        self.grad.append(np.dot(extend_X.T,extend_grad))#weight的偏导，10*20
        self.grad.append(extend_grad.sum(axis=0))#bias的偏导，1*20
        extend_res = np.dot(extend_grad, self.params[0].T)#x的偏导，展开，15*10
        return extend_res.reshape(*X.shape)#还原回3*5*10
        raise NotImplementedError
...
class NLLLoss(Node):
    ...
    def cal(self, X):
        ...
        #把y扩充成（*，d)与X的维度对齐，多出来的维度不填充，只做维度对齐
        #沿着X的最后一个维度（类别维度）取y中的对应索引
...
class CrossEntropyLoss(Node):
    ...
    def backcal(self, grad):
        ...
        X, y = self.cache[-1], self.y
        ret = np.zeros_like(X)
        np.put_along_axis(ret, np.expand_dims(y, axis=-1), -1, axis=-1) #把y扩充，在ret中目标类填-1
        ret = ret/ (np.take_along_axis(X, np.expand_dims(y,axis=-1), axis=-1)) #ret对应位置除以取出填了-1的类的X值
        return grad * ret
        raise NotImplementedError