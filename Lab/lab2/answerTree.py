...
def entropy(Y: np.ndarray):
    ...
    @return: 熵
    """
    u,cnt = np.unique(Y,return_counts=True)#不重复的值列表，出现次数
    n = Y.shape[0]
    H = (-cnt/n*np.log2(cnt/n)).sum()
    return H
...
def buildTree(X: np.ndarray, Y: np.ndarray, unused: List[int], depth: int, purity_bound: float, gainfunc: Callable, prefixstr=""):
    ...
    @params depth: int, 树的当前深度(剩余允许探索的深度)
...
def inferTree(root: Node, x: np.ndarray):
    ...
    if root.isLeaf():
        return root.label
    child = root.children.get(x[root.featidx], None)#说明每个children的key是属性值！！
